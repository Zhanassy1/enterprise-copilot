...
- Prometheus-like metrics endpoint: `/metrics` (HTTP counters and latency sums; при `observability_metrics_enabled`): также счётчики Celery (`celery_ingestion_terminal_failures_total`, `celery_ingestion_retries_total`) и агрегаты `ingestion_jobs_total{status="..."}` по таблице `ingestion_jobs` — см. `backend/app/main.py` (`metrics()`).
