...
Для маршрутов с заголовком **`X-Workspace-Id`** неявного выбора workspace нет: без заголовка API отвечает **400**. Список доступных workspace — **`GET /workspaces`**; дальше клиент обязан передавать выбранный UUID в заголовке.
