...
Уже есть: **workspace switcher** (карточка текущего **рабочего пространства**, бейдж роли — `workspace-switcher.tsx`), **NavRoleHint** в сайдбаре/моб. меню для **viewer**, **landing** с блоком «демо за 1 минуту», страницы **документы / поиск / чат / команда / billing / jobs / audit**, контекстные плашки workspace, **WorkspaceViewerBanner** на документах, поиске, jobs. **`/team`:** матрица ролей, строка «Вы» в таблице участников, пустая таблица приглашений, неактивная кнопка «Отправить приглашение» для owner/admin до API.
