...
Уже есть: **workspace switcher** (карточка текущего workspace, бейдж роли, восстановление при битом id — `workspace-provider.tsx` + `workspace-switcher.tsx`), **landing** с якорем «демо за 1 минуту», страницы **документы / поиск / чат / команда / billing / jobs / audit**, контекстные плашки «данные этого workspace», **role-aware** действия для **viewer**: дизейбл загрузки, удаления, новых чатов и отправки сообщений + **WorkspaceViewerBanner** на документах, поиске, очереди jobs. **`/team`:** ролевые подсказки, неактивная кнопка «Пригласить» для owner/admin (честный placeholder до API), без фейковой таблицы участников.
...
2. **Нет API для списка участников / приглашений** — только заготовка на `/team`.
3. **Нет широкого e2e-покрытия** фронта (Playwright; `demo:screenshots` — съёмка UI).
