...
3. **Worker** извлекает текст, режет на chunks; персистенция чанков и pgvector — через **`DocumentChunkRepository`** (`app/repositories/document_chunks.py`): пакетная вставка с `embedding_vector` = NULL, **коммит**, затем embeddings **батчами** (`EMBEDDING_BATCH_SIZE`, коммит после каждого батча). При сбое повторная задача может **дозаполнить** только NULL-векторы без повторного PDF/OCR, если не менялись `CHUNK_SIZE`/`CHUNK_OVERLAP` (fingerprint в `documents.extraction_meta.indexing`). Размерность колонки `vector` в БД и модели эмбеддинга должны совпадать (сейчас 384; смена модели — отдельная миграция). Если воркер исчез с `ingestion_jobs` в `processing` (старый `locked_at`) или повтор не дошёл до брокера, **beat**-задача `maintenance.requeue_stale_ingestion_jobs` (раз в 5 минут) безопасно возвращает job в `queued` и снова ставит `ingest_document_task` в очередь при наличии NULL-embeddings; gauge **`documents_with_null_embeddings`** в `/metrics` — контроль незавершённых чанков.
...
Слой **generic hybrid** (`app/services/retrieval/generic_hybrid.py`): dense pgvector + полнотекст (tsvector) + RRF (или опционально **weighted_scores** — min-max по спискам и смесь с `retrieval_score_fusion_alpha`); размер кандидат-пула и веса RRF могут подстраиваться по **типу запроса** (`app/services/retrieval/tuning.py`, JSON-оверрайды `retrieval_query_kind_policy_json` в `LLMSettings`). SQL к таблице `document_chunks` выполняется в **`DocumentChunkRepository`** (тот же модуль репозитория). Полнотекст **не** считает `to_tsvector` по `text` на лету: в таблице хранятся сгенерированные столбцы `chunk_tsv_ru`, `chunk_tsv_simple`, `chunk_tsv_aux` (последний — от поля `chunk_search_aux` с токенами артикулов/кодов, заполняется при индексации), с **GIN**-индексами; отбор кандидатов идёт через предикат `@@`, ранжирование — `ts_rank_cd` по сохранённым векторам. Разбор запроса: `websearch_to_tsquery` с fallback на `plainto_tsquery`, для «кодовых» однострочных запросов — `phraseto_tsquery` на `simple` (см. `app/services/retrieval/keyword_query.py`). Поверх него **domain rules** (`app/services/retrieval/domain_rules.py`): эвристики по intent (цена / неустойка / расторжение и т.д.), порог `retrieval_min_score`, dedup почти дубликатов; веса вынесены в `RetrievalRuleWeights` (`app/core/settings/retrieval_rules.py`, поле `retrieval_domain_rules` в настройках). Далее общий пайплайн RAG в `rag_retrieval.py`: опциональный cross-encoder rerank (параметры инференса — device, batch, max length и таймаут predict — в `LLMSettings`; latency и таймауты в `/metrics`), compaction сниппетов, пост-правила для «цена договора» (`nlp.py`).

**Offline eval:** конфиг `backend/eval/retrieval_eval.config.json`; скрипты `backend/scripts/eval_retrieval.py` (вектор + baseline; опционально `--overrides-json`), `backend/scripts/tune_retrieval.py` (grid/random по train/holdout), и `backend/scripts/eval_rag_quality.py` (единый JSON: вектор, ranked rerank-off, answer/citation метрики, опционально `--rerank-gain`). Синтетический gold (без ПДн) — политика и запуск: [backend/eval/README.md](../backend/eval/README.md); стратификация метрик по `query_type`/`tags` в gold. Регрессия: `pytest` с маркером `retrieval_regression` при `RUN_INTEGRATION_TESTS=1` (retrieval + answer quality + rerank stub; входит в CI job с Postgres). Опционально реальный cross-encoder: `RUN_RERANK_EVAL=1` (медленно, не в обязательном PR job).
