...
В **production** индексация только через worker (`INGESTION_ASYNC_ENABLED=1`). Синхронная индексация на upload допустима **только** в `ENVIRONMENT=local` с `ALLOW_SYNC_INGESTION_FOR_DEV=1` и `INGESTION_ASYNC_ENABLED=0` (например интеграционные тесты без Celery).
...
Второй шаг — async ingestion pipeline (в деплое это единственный поддерживаемый путь; см. `docker compose` worker и `INGESTION_ASYNC_ENABLED`).
